#--------------------------------------------------------------------------------------------------
# Move various files over biadwiki for hosting, using REST API
# Many files (images, csv templates, html, etc) are generated each night by the server.
# Rather than gumming up the github repository with so many changes, 
# these files are sent to biadwiki.org to be used as assets, directly embedded into webpages.
#--------------------------------------------------------------------------------------------------
require(httr)
require(tools)
#--------------------------------------------------------------------------------------------------
...
path <- c(list.files('../tools/html', full.names=TRUE),path)
